...
- Steps:
  ...
  2. `aquasecurity/trivy-action@v0.36.0` — `scan-type: fs`, `scanners: vuln,secret`, `format: sarif`, `output: trivy-fs.sarif`, `exit-code: 0` (advisory), `ignore-unfixed: true`.
  3. `github/codeql-action/upload-sarif@v3` — `sarif_file: trivy-fs.sarif`, `category: trivy-fs`, `if: always()`.
  4. `aquasecurity/trivy-action@v0.36.0` — `scan-type: config`, `format: sarif`, `output: trivy-config.sarif`, `exit-code: 0`.
